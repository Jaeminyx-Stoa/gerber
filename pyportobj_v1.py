...
def std_objective(x, pars):
    ...
    sig_p = np.sqrt(x@V@x.T) #portfolio standard deviation
    return sig_p + PEN * np.sum(x**2)

def return_objective(x, pars):
    R = pars[0]
    PEN = pars[1]
    r_p = R @ x
    return -r_p + PEN * np.sum(x**2)
...
def drawdown_objective(x, pars):
    ...
    PEN = pars[1]
    pf_r = returns @ x
    return -min(np.cumprod(pf_r+1)/np.maximum.accumulate(np.cumprod(pf_r+1))-1) + PEN * np.sum(x**2)
    
def skewness_objective(x, pars):
    returns = pars[0]
    PEN = pars[1]
    pf_r = returns @ x
    return -pf_r.skew() + PEN * np.sum(x**2)
...
def varisk_objective(x, pars):
    ...
    PEN = pars[1]
    pf_r = returns @ x
    percentile = pars[1]  + PEN * np.sum(x**2)# ex. 0.05
...
def cvarisk_objective(x, pars):
    ...
    return - pf_r[pf_r<=pf_r.quantile(0.01)].mean()  + PEN * np.sum(x**2)