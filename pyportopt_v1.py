...
class WeightOptimizer:
    ...
    def __init__(self, cov=None, returns=None,
                 ...
                 custom_range_dict = None, turnover_pen = 0,
        ...
        self.custom_range_dict = custom_range_dict
        
        self.tolcon = tolcon
        self.display = display
        
        self.turnover_pen = turnover_pen
        
    def constraint_maker(self, key=''):
        def constraint(x):
            lower = self.custom_range_dict[key][0][0]
            upper = self.custom_range_dict[key][0][1]
            mask = self.custom_range_dict[key][1]
            return -np.abs(x@mask - (upper+lower)/2)+(upper-lower)/2
        return constraint
    ...
    def custom_weight(self, objective, add=None):
        ...
        if self.gross_weight != None:
            ...
            cons = ({'type': 'eq', 'fun': lambda x: np.sum(x)-self.net_weight})               
        if self.custom_range_dict:
            cons = [cons]
            for key in self.custom_range_dict.keys():
                cons.append({'type':'ineq','fun': self.constraint_maker(key)})
            cons = tuple(cons)
    ...
    def markowitz_weight_bnd_sr(self):
        ...
        res = minimize(sharpe_objective, w0, args=[R,V, self.turnover_pen], method='SLSQP',
                  constraints=cons, bounds = bounds, options={'disp': False},
                  tol=self.tolcon)
        if self.display and not res.success:
            print(res.message)
    ...
    def markowitz_weight_bnd_mu(self, target_mu):
        ...
        res= minimize(std_objective, w0, args=[V, self.turnover_pen], method='SLSQP',
                  constraints=cons, bounds = bounds, options={'disp': False},
                  tol=self.tolcon)
        if self.display and not res.success:
            print(res.message)
    ...
    def markowitz_weight_bnd_std(self, target_std):
        ...
        res= minimize(return_objective, w0, args=[R, self.turnover_pen], method='SLSQP',
                  constraints=cons, bounds = bounds, options={'disp': False},
                  tol=self.tolcon)
        if self.display and not res.success:
            print(res.message)
    ...
    def riskparity_weight(self, risk_weight):
        ...
        res= minimize(risk_budget_objective, w0, args=[V,risk_weight, self.turnover_pen], method='SLSQP',
                  constraints=cons, bounds = bounds, options={'disp': False},
                  tol=self.tolcon)
    ...
    def md_weight(self):
        ...
        res= minimize(diversify_objective, w0, args=[V, self.turnover_pen], method='SLSQP',
                  constraints=cons, bounds = bounds, options={'disp': False},
                  tol=self.tolcon)
        if self.display and not res.success:
            print(res.message)
